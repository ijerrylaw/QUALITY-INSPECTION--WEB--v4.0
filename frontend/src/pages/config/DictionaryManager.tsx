    ...
    <div className="bg-canvas border border-gray-800 rounded-xl overflow-hidden shadow-sm flex flex-col h-[400px]">
      ...
      <div className="p-2 flex-1 overflow-y-auto">
        <div className="flex flex-col">
          {options.map((opt, idx) => (
            <div key={opt.value} className="py-3 px-3 border-b border-gray-700/50 flex flex-col justify-center group relative hover:bg-surface/50 transition-colors">
              {editingVal === opt.value ? (
                <div className="flex items-start gap-1 py-1">
                  <div className="flex flex-col gap-1 w-full min-w-0">
                    <input 
                      ...
                      className="w-full h-9 px-2 rounded bg-canvas border border-gray-700 text-brand-secondary font-bold font-mono text-sm focus:border-brand-secondary focus:ring-1 focus:ring-brand-secondary outline-none uppercase"
                    />
                    <input 
                      type="text" 
                      value={editInputLbl} 
                      ...
                      className="w-full h-9 px-2 rounded bg-canvas border border-gray-700 text-primary font-mono text-sm focus:border-brand-secondary focus:ring-1 focus:ring-brand-secondary outline-none"
                    />
                  </div>
                ...
                <div className="flex items-center w-full">
                  <div className="flex flex-col gap-0.5 overflow-hidden w-full py-0.5">
                      <span className="font-mono text-sm font-bold text-brand-secondary shrink-0">{opt.value}</span>
                      <span className="font-mono text-sm text-primary truncate leading-tight pr-6">{opt.label}</span>
                  </div>
          ...
          {isAdding && (
            <div className="py-3 px-3 border-b border-gray-700/50 flex items-start gap-1">
              <div className="flex flex-col gap-1 w-full min-w-0">
                <input
                  ref={valInputRef}
                  type="text"
          ...
          {options.length === 0 && !isAdding && (
            <div className="text-xs text-muted italic text-center py-2">No entries</div>
          )}
        </div>
        ...
        {!isAdding && (
          <button
            ...
            className="w-full mt-2 h-8 rounded border border-dashed border-gray-700 bg-transparent text-muted hover:text-brand-secondary hover:border-brand-secondary/50 hover:bg-brand-primary/10 font-semibold text-[11px] uppercase tracking-wider flex items-center justify-center gap-1 transition-all outline-none"
          >
