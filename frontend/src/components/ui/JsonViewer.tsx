 ...
 * this covers the one use case — rendering a raw object/array value inline,
 * e.g. AmendmentDiffView.tsx's per-section "unchanged fields" panel and its
 * fallback for container-shaped leaf values — without adding one).
 *
 ...
 *
 * The diff-aware tree renderer that used to live in this file
 * (`DiffJsonViewer`/`DiffTreeNode`) was removed when ApprovalsQueue.tsx's
 * two-column red/green JSON diff was replaced by AmendmentDiffView.tsx's
 * flattened, section-grouped layout — `tryParseJSON` below is still used by
 * `diffTree.ts`'s normalization step, and plain `JsonViewer` is still used
 * for container-shaped values, but the DiffNode-walking renderer had no
 * remaining callers.
 */
