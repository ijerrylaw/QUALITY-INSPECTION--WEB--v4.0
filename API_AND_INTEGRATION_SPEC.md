...
* `POST /api/submissions`
  ...
  * **Important:** `profileId` is stored as a plain opaque string, checked against the `Profile` table/the `'prof_default'` sentinel (not a Prisma foreign key on `Submission` — just a same-request sanity check, `submissions.routes.ts`'s `isKnownProfileId()`; profile IDENTITY has lived on the `Profile` table since Stage A0, not `AppConfig.inspectionProfiles`, which no longer exists as a column — AUDIT_REPORT.md #37). If a profile *was* resolved for grading but the requested id fails this sanity re-check, the warning is logged and `profileId` is stored as `null` rather than blocking the write.
  * **Response 404 (`{ error }`) — explicit but unrecognized `profileId`:** if the payload supplies a non-empty `profileId` that matches no `Profile` table entry and is not the `'prof_default'` sentinel, `resolveVerdict()` (default `onUnresolvedProfile: 'throw'`) raises `VerdictProfileNotFoundError`; the route returns 404 and **no `Submission` row is created**. This is **by design, not a gap** (AUDIT_REPORT.md #11): a submission is a permanent record, so it must never be silently graded against a fallback profile the caller did not ask for — failing loudly is preferable. Contrast `POST /api/verdict/preview`, which runs in `'fallback'` mode and degrades a bad `profileId` to the safety-net profile with a 200: acceptable there because preview results are disposable and never persisted. An **absent or empty** `profileId` is not a "miss" — it triggers the normal resolution order above, not this 404.
...
* `POST /api/submissions/:id/amendments`
  ...
  * **Payload:** `{ reason: string, reasonCode?: AmendmentReasonCode, acknowledgedChanges?: string[], newValues: Partial<Submission> }`
  * **Note:** Does NOT re-evaluate the AQL verdict. The verdict in `newValues` is whatever the caller supplies.
  * **`reasonCode` (optional):** one of `WRONG_INSPECTION_PROFILE` | `RECOUNT_OR_MISCOUNTED_DEFECT` | `DATA_ENTRY_CORRECTION` | `OTHER` (`backend/src/lib/amendmentReason.ts`). An **audit field only** — never compared against the diff. Validated **only when present**: an unrecognized value is a `400`, absence is accepted, because the dropdown that supplies it ships with the amendment-acknowledgment UI. `reason` remains the required free note. **Response 400:** `{ error, allowedReasonCodes }`
  * **Acknowledged-changes gate:** the server independently recomputes what this amendment changes (`backend/src/lib/amendmentDiff.ts`) and rejects any change the requester did not acknowledge. Exists because the stated reason had no structural link to the payload — on lot A001A6247003 the note read "wrong inspection profile" while the same request also raised two defect counts from 0 to 1. **Response 400:** `{ error, unacknowledgedChanges: AmendmentChange[], detectedChangeCount }`, where `AmendmentChange` is `{ path, changeType: 'modified' | 'added' | 'removed', from, to }`.
    * **`path` is a wire contract** — it is the acknowledgment key the client echoes back. One entry per scalar field (`"profileId"`), per defect (`"defects.<defectId>"`, absent ≡ count 0), and per dimension (`"dimensions.<dimId>"`, whole 5-slot array — not per slot). `dimensionMins` is excluded as derived from `dimensions`; `verdict` is deliberately included.
    * **Enforced only when `acknowledgedChanges` is present.** Absent ⇒ a pre-gate client (including the current wizard until its checkbox UI ships); the draft proceeds and the column is stored `NULL`. An empty array is **not** absence — that is a gate-aware client reporting nothing to acknowledge, and it is enforced. An explicit JSON `null`, or any non-string-array, is a `400`.
    * **Rejects before any write:** the gate runs ahead of the recompute preview and the transaction, so a rejected amendment leaves `amendmentStatus` untouched and creates no `AmendmentLog` row.
    * **Grandfathering is structural, not a date cutoff.** The gate lives only in this route, at creation. `POST /api/amendments/:id/approve` never re-validates a payload, so already-PENDING drafts can never fail it retroactively. It also never applies to `POST /api/submissions` — a new submission has no prior record to diff against.
