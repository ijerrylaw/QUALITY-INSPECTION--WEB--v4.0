...
* Resolves the authenticated user's role by checking their Entra ID group membership against the configured security group, then maps that to `role` (`ADMIN` / `MANAGER` / `SUPERVISOR` — `LEADER`/`OPERATOR`/`INTERN` are never resolved this way and get PIN login instead).
