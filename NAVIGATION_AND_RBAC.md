...
The same lock rule reaches `PATCH /api/config`: a Quality Rules save that would drop a locked defect/category from its last remaining profile is rejected there too. That write and its grading-table projection run in **one transaction** — a rejected save rolls back completely and changes nothing (`API_AND_INTEGRATION_SPEC.md` §1). The rejected attempt is still recorded in the audit trail as an **`AccessLog` row with `action: 'CONFIG_WRITE_FAILURE'`** — distinct from the `CONFIG_WRITE` written on success, since nothing was persisted. `AccessLogPanel.tsx` renders it red via the shared `_FAILURE` suffix convention.
