...
* **`ADD` means "create a new entity" — do not reuse it for "pick an existing one".** The `+ ADD CATEGORY` / `+ ADD DEFECT` header buttons in `QualityRules.tsx` §3.5 are true create actions: they open the global Category Inventory / Master Defect List registry where a brand-new entry is registered system-wide. The look-alike picker modals reached from the §3.6 dashed buttons (`CategoryPickerModal.tsx` / `DefectPickerModal.tsx`) do NOT create anything — they only adopt an existing global entry into the active profile — so their titles read `SELECT CATEGORY` / `SELECT DEFECT`, not `ADD …`, and they are not an instance of this "create" pattern even though they borrow the emerald ghost-outline styling for their internal `REGISTER NEW …` button.
