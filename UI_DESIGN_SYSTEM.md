...
* **Purpose:** Emerald ghost-outline buttons that sit outside inline forms — in card headers or action bars — for a top-level action on an entity or collection (e.g. `+ ADD PROFILE`, `+ ADD PRODUCT CODE`, `MANAGE CATEGORIES`).
* **Naming Standard:** use the verb that matches what the button actually does; never `NEW`, `CREATE`, or `INSERT`.
  * `+ ADD [ENTITY]` — mints a new top-level entity directly (`+ ADD PROFILE`, `+ ADD PRODUCT CODE`) **or**, in the Category/Defect flows below, attaches an entry that already exists in a global registry to the active profile.
  * `MANAGE [PLURAL]` — opens a management/registry view; does not itself create or attach anything.
* **Category & Defect flows — three verbs, kept deliberately distinct** (`QualityRules.tsx` §3.5/§3.6, `RegistryManagerModal.tsx`, `CategoryPickerModal.tsx`, `DefectPickerModal.tsx`):
  * **MANAGE** — the `MANAGE CATEGORIES` / `MANAGE DEFECTS` header buttons open `RegistryManagerModal` (the Category Inventory / Master Defect List). Browse, rename, and register global entries here; nothing is attached to a profile from this view. (Formerly mislabelled `+ ADD CATEGORY` / `+ ADD DEFECT`, which wrongly implied a create action — corrected 2026-09-06.)
  * **REGISTER** — `REGISTER CATEGORY` / `REGISTER DEFECT` is the canonical "create a brand-new global entry" action. Worded identically whether reached inside `RegistryManagerModal` directly or via a picker's create-handoff button — no `NEW` (the picker buttons dropped it 2026-09-06). This, not `+ ADD CATEGORY`, is the reference example for the create pattern.
  * **ADD** — `ADD CATEGORY` / `ADD DEFECT` (picker modal `<h3>`), the `+ ADD` dashed openers (§3.6), each picker's per-row and confirm buttons, and the "N … available to add" footer count all mean "attach an entry that already exists in the registry to the active profile". They never create.
